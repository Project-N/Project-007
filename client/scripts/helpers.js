...
Template.viewport.helpers({
    ...
    youx: function () {
        ...
        if (locked) {
            ...
            return getx(Session.get('lastPos').lat, Session.get('lastPos').long, Players.findOne({
                id: Session.get('id')
            }).position.latitude, Players.findOne({
                id: Session.get('id')
            }).position.longitude, zoom) - 13;
        }
    },
    rotate: function () {
        if (Session.get('locked') == true) {
        ...
        if (locked) {
            ...
            if (typeof lastPos == 'undefined') {
                ...
                lastPos = {
                    ...
                    'long': player.position.longitude
                };
            }
            var zoom = Session.get('zoom');
            if (typeof zoom == 'undefined') {
...
Template.dot.helpers({
    x: function () {
        if (Session.get('locked') == true) {
            ...
            return getx(Players.findOne({
                id: Session.get('id')
            }).position.latitude, Players.findOne({
                id: Session.get('id')
            }).position.longitude, this.position.latitude, this.position.longitude, zoom) - 13;
        } else {
            var zoom = Session.get('zoom');
            if (typeof zoom == 'undefined') {
                zoom = 0.5;
    ...
    yu: function () {
        if (Session.get('locked') == true) {
            ...
            return gety(Players.findOne({
                ...
                id: Session.get('id')
            }).position.longitude, this.position.latitude, this.position.longitude, zoom);
        } else {
            var zoom = Session.get('zoom');
            if (typeof zoom == 'undefined') {
                zoom = 0.5;
